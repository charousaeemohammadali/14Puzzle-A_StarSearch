...
listnode* delete_min_frontier(node* minimum);
listnode* delete_repeated_frontier(node* minimum);
node* min_frontier();
listnode* add_explored(node*);
void print_output(node* temp);
int manhatan(int[4][4]);
node* find_node_frontier(listnode*, node*);
...
int main()
{
	...
	while (true)
	{
		...
		//cout << "manhatan = " << manhatan(frontier->n->s) << endl;
		...
		frontier = delete_min_frontier(temp);
...
{
	node* n = new node;
	int matrix[4][4] = { { 0, 1, 2, 3 },{ 5,6,7,4 },{ 10,11,14,8 },{ 9,13,0,12 } };
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < 4; j++)
			n->s[i][j] = matrix[i][j];
	n->g = 0;
...
void move_up(node* p, int location[2])
{
	...
	child->h = manhatan(child->s);
	child->f = child->g + child->h;

	if (!search(child, frontier) && !search(child, explored))
		frontier = insert(frontier, child);
	else if (search(child, frontier))
	{
		node* mynode;
		mynode = find_node_frontier(frontier, child);
		if (mynode->f <= child->f)
		{
			delete child;
			return;
		...
		{
			delete_repeated_frontier(mynode);
			frontier = insert(frontier, child);
		}
...
listnode* delete_min_frontier(node* minimum)
{
	if (compare(frontier->n->s, minimum->s))
	{
		listnode* q = frontier;
	...
	{
		listnode* p = frontier;
		listnode* q = frontier->next;
		while (q != NULL)
		{
			if (compare(q->n->s, minimum->s))
			{
				p->next = q->next;
				delete q;
				return frontier;
...
listnode* delete_repeated_frontier(node* repeated)
{
	if (compare(frontier->n->s, repeated->s))
	{
		listnode* q = frontier;
		frontier = frontier->next;
		delete q->n;
		...
		for (int j = 0; j < 4; j++)
		{
			if (matrix[i][j] != 0)
			{
				for (int k = 0; k < 4; k++)
				{
					for (int l = 0; l < 4; l++)
					{
						if (matrix[i][j] == goal_matrix[k][l])
							manhatan_dist += abs(i - k) + abs(j - l);
					}
				}
			}
		}
...
node* find_node_frontier(listnode* list, node* mynode)
{
	listnode* temp = list;
	while (temp)
	{
		if (!compare(temp->n->s, mynode->s))
			temp = temp->next;
		else
			break;
	}
